...
from hummingbot.strategy_v2.strategy_frameworks.data_types import (
    OrderLevel,
    TripleBarrierConf,
)
from hummingbot.strategy_v2.strategy_frameworks.directional_trading import (
...
def get_databases():
    sqlite_files = [
        db_name for db_name in os.listdir(BASE_DATA_DIR) if db_name.endswith(".db")
    ]
    databases_list = [
        OptunaDBManager(db, db_root_path=BASE_DATA_DIR) for db in sqlite_files
    ]
...
if not dbs:
    ...
    with filters_column:
        accuracy = st.slider(
            "Accuracy", min_value=0.0, max_value=1.0, value=[0.4, 1.0], step=0.01
        )
        net_profit = st.slider(
            "Net PNL (%)",
            min_value=merged_df["net_pnl_pct"].min(),
            max_value=merged_df["net_pnl_pct"].max(),
            value=[merged_df["net_pnl_pct"].min(), merged_df["net_pnl_pct"].max()],
            step=0.01,
        )
        max_drawdown = st.slider(
            "Max Drawdown (%)",
            min_value=merged_df["max_drawdown_pct"].min(),
            max_value=merged_df["max_drawdown_pct"].max(),
            value=[
                merged_df["max_drawdown_pct"].min(),
                merged_df["max_drawdown_pct"].max(),
            ],
            step=0.01,
        )
        total_positions = st.slider(
            "Total Positions",
            min_value=merged_df["total_positions"].min(),
            max_value=merged_df["total_positions"].max(),
            value=[
                merged_df["total_positions"].min(),
                merged_df["total_positions"].max(),
            ],
            step=1,
        )
        net_profit_filter = (merged_df["net_pnl_pct"] >= net_profit[0]) & (
            merged_df["net_pnl_pct"] <= net_profit[1]
        )
        accuracy_filter = (merged_df["accuracy"] >= accuracy[0]) & (
            merged_df["accuracy"] <= accuracy[1]
        )
        max_drawdown_filter = (merged_df["max_drawdown_pct"] >= max_drawdown[0]) & (
            merged_df["max_drawdown_pct"] <= max_drawdown[1]
        )
        total_positions_filter = (
            merged_df["total_positions"] >= total_positions[0]
        ...
        if field_name not in ["candles_config", "order_levels", "position_mode"]:
            ...
            if field_name == "candles_config":
                ...
                for i, candles_config in enumerate(field_value):
                    ...
                    with c11:
                        connector = st.text_input(
                            "Connector", value=candles_config["connector"]
                        )
                    with c12:
                        trading_pair = st.text_input(
                            "Trading pair", value=candles_config["trading_pair"]
                        )
                    with c13:
                        interval = st.text_input(
                            "Interval", value=candles_config["interval"]
                        )
                    with c14:
                        max_records = st.number_input(
                            "Max records", value=candles_config["max_records"]
                        )
                    st.write("---")
                    candles.append(
                        CandlesConfig(
                ...
                for order_level in field_value:
                    st.write(
                        f"### Level {order_level['level']} {order_level['side'].name}"
                    )
                    ...
                    with ol_c1:
                        ...
                        with c23:
                            time_limit = st.number_input(
                                "Time Limit",
                                value=triple_barrier_conf_level["time_limit"],
                                key=f"{order_level['level']}_{order_level['side'].name}_tl",
                            )
                        with c24:
                            ts_ap = st.number_input(
                                "Trailing Stop Activation Price",
                                value=float(
                                    triple_barrier_conf_level[
                        ...
                        with c25:
                            ts_td = st.number_input(
                                "Trailing Stop Trailing Delta",
                                value=float(
                                    triple_barrier_conf_level[
                    ...
                    with ol_c2:
                        ...
                        with c32:
                            cooldown_time = st.number_input(
                                "Cooldown time",
                                value=order_level["cooldown_time"],
    ...
    with col2:
        initial_portfolio_usd = st.number_input(
    ...
    if run_backtesting_button:
        try:
            ...
            metrics_container = BacktestingGraphs(
                backtesting_results["processed_data"]
            ).get_trial_metrics(
                strategy_analysis, add_positions=add_positions, add_volume=add_volume
            )
