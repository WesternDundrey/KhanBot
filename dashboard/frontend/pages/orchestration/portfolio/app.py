...
accounts = st.multiselect(
    "Select Accounts", list(account_state.keys()), list(account_state.keys())
)
if len(accounts) == 0:
...
for account in accounts:
    for exchange in exchanges:
        if exchange in account_state[account]:
            tokens_available += [
                info["token"] for info in account_state[account][exchange]
            ]
    ...
    for exchange in exchanges:
        if exchange in account_state[account]:
            filtered_account_state[account][exchange] = [
                token_info
                for token_info in account_state[account][exchange]
            ...
            for exchange in exchanges:
                if exchange in record["state"][account]:
                    filtered_record["state"][account][exchange] = [
                        token_info
                        for token_info in record["state"][account][exchange]
                        if token_info["token"] in tokens_available
                    ]
...
if len(filtered_account_state) > 0:
    ...
    with c2:
        account_state_df["% Allocation"] = (
            account_state_df["value"] / total_balance_usd
        ) * 100
        account_state_df["label"] = (
            account_state_df["token"]
            + " ($"
            + account_state_df["value"].apply(lambda x: f"{x:,.2f}")
            + ")"
        )
    ...
    st.dataframe(
        account_state_df[
            ["exchange", "token", "units", "price", "value", "available_units"]
        ],
        width=1800,
        height=600,
    )
...
if len(filtered_account_history) > 0:
    ...
    portfolio_evolution_df = (
        account_history_df.groupby("timestamp")["value"].sum().reset_index()
    )
    ...
    token_evolution_df = (
        account_history_df.groupby(["timestamp", "token"])["value"].sum().reset_index()
    )
    ...
    fig = px.area(
        ...
        y="value",
        color="token",
        title="Token Value Evolution Over Time",
        color_discrete_sequence=px.colors.qualitative.Vivid,
    )
