...
def update_active_bots(api_client):
    ...
    if active_bots_response.get("status") == "success":
        ...
        stored_bots = {
            card[1]: card for card in st.session_state.active_instances_board.bot_cards
        }
        ...
        for bot in removed_bots:
            st.session_state.active_instances_board.bot_cards = [
                card
                for card in st.session_state.active_instances_board.bot_cards
                if card[1] != bot
            ]
        positions = get_grid_positions(
            len(current_active_bots), NUM_CARD_COLS, CARD_WIDTH, CARD_HEIGHT
        )
        for bot, (x, y) in zip(new_bots, positions[: len(new_bots)]):
            card = BotPerformanceCardV2(
                st.session_state.active_instances_board.dashboard,
                x,
                y,
                CARD_WIDTH,
                CARD_HEIGHT,
...
if "active_instances_board" not in st.session_state:
    ...
    if number_of_bots > 0:
        positions = get_grid_positions(
            number_of_bots, NUM_CARD_COLS, CARD_WIDTH, CARD_HEIGHT
        )
