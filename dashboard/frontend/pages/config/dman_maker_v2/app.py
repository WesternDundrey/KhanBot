...
# Page content
st.text(
    "This tool will let you create a config for D-Man Maker V2 and upload it to the BackendAPI."
)
...
c1, c2 = st.columns(2)
with c1:
    executor_level = st.selectbox(
        "Executor Level", buy_executor_levels + sell_executor_levels
    )
    side, level = executor_level.split("_")
    if side == "BUY":
        dca_amount = (
            inputs["buy_amounts_pct"][int(level)] * inputs["total_amount_quote"]
        )
    else:
        dca_amount = (
            inputs["sell_amounts_pct"][int(level)] * inputs["total_amount_quote"]
        )
with c2:
