    ...
    with st.expander("SuperTrend Configuration", expanded=True):
        ...
        with c1:
            length = st.number_input(
                "Supertrend Length", min_value=1, max_value=200, value=length
            )
        with c2:
            multiplier = st.number_input(
                "Supertrend Multiplier", min_value=1.0, max_value=5.0, value=multiplier
            )
        with c3:
            percentage_threshold = (
                st.number_input("Percentage Threshold (%)", value=percentage_threshold)
                / 100
            )
    return {
