...
initialize_st_page(
    title="Kalman Filter V1", icon="📈", initial_sidebar_state="expanded"
)
...
def get_candles(
    ...
    return backend_client.get_real_time_candles(
        connector_name, trading_pair, interval, max_records
    )
...
def add_indicators(
    ...
    df["kf_upper"] = pd.Series(
        mean.flatten() + 1.96 * cov.flatten(), index=df["close"].index
    )
    df["kf_lower"] = pd.Series(
        mean.flatten() - 1.96 * cov.flatten(), index=df["close"].index
    )
...
st.text(
    "This tool will let you create a config for Kalman Filter V1 and visualize the strategy."
)
st.write("---")
...
with c3:
    interval = st.selectbox(
        "Candle Interval", options=["1m", "3m", "5m", "15m", "30m"], index=1
    )
with c4:
    max_records = st.number_input(
        "Max Records", min_value=100, max_value=10000, value=1000
    )
...
with c1:
    sl = st.number_input(
        "Stop Loss (%)", min_value=0.0, max_value=100.0, value=2.0, step=0.1
    )
    tp = st.number_input(
        "Take Profit (%)", min_value=0.0, max_value=100.0, value=3.0, step=0.1
    )
    take_profit_order_type = st.selectbox(
        "Take Profit Order Type", (OrderType.LIMIT, OrderType.MARKET)
    )
with c2:
    ...
    ts_delta = st.number_input(
        "Trailing Stop Delta (%)", min_value=0.0, max_value=100.0, value=0.3, step=0.1
    )
    time_limit = st.number_input("Time Limit (minutes)", min_value=0, value=60 * 6)
with c3:
    executor_amount_quote = st.number_input(
        "Executor Amount Quote", min_value=10.0, value=100.0, step=1.0
    )
    max_executors_per_side = st.number_input(
        "Max Executors Per Side", min_value=1, value=2
    )
...
with c2:
    transition_covariance = st.number_input(
        "Transition Covariance", value=0.001, step=0.0001, format="%.4f"
    )
...
fig = make_subplots(
    ...
    subplot_titles=("Candlestick with Kalman Filter", "Trading Signals"),
    row_heights=[0.7, 0.3],
)
...
fig.add_trace(
    go.Scatter(
        x=candles_processed.index,
        y=candles_processed["kf"],
        line=dict(color=tech_colors["middle_band"]),
        name="Middle Band",
    ),
    row=1,
    col=1,
...
fig.add_trace(
    go.Scatter(
        ...
        mode="markers",
        marker=dict(
            color=signals["signal"].map(
                {1: tech_colors["buy_signal"], -1: tech_colors["sell_signal"]}
            ),
            size=10,
        ),
        showlegend=False,
...
fig.update_xaxes(
    rangeslider_visible=False, row=1, col=1  # Disable range slider for all
)
...
with c1:
    config_base = st.text_input(
        "Config Base",
        value=f"bollinger_v1-{connector_name}-{trading_pair.split('-')[0]}",
    )
with c2:
