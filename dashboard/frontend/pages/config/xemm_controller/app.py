...
# Page content
st.text(
    "This tool will let you create a config for XEMM Controller and upload it to the BackendAPI."
)
st.write("---")
...
with c3:
    min_profitability = (
        st.number_input("Min Profitability (%)", value=0.2, step=0.01) / 100
    )
    max_profitability = (
        st.number_input("Max Profitability (%)", value=1.0, step=0.01) / 100
    )
with c4:
    ...
    for i in range(buy_maker_levels):
        with c41:
            target_profitability = st.number_input(
                f"Target Profitability {i + 1} B% ", value=0.3, step=0.01
            )
        with c42:
...
with c5:
    ...
    for i in range(sell_maker_levels):
        with c51:
            target_profitability = st.number_input(
                f"Target Profitability {i + 1}S %", value=0.3, step=0.001
            )
        with c52:
...
def create_order_graph(order_type, targets, min_profit, max_profit):
    ...
    x_labels = [
        f"{x:.2f}%" for x in x_values
    ]  # Format x labels as strings with percentage sign
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            range=[
                0,
                max(max(x_values + [min_profit_percent, max_profit_percent]) + 0.1, 1),
            ],
            # Adjust range to include a buffer
        ),
        yaxis=dict(title="Order Amount"),
        height=400,
        width=600,
    )
...
buy_order_fig = create_order_graph(
    "buy", buy_targets_amounts, min_profitability, max_profitability
)
sell_order_fig = create_order_graph(
    "sell", sell_targets_amounts, min_profitability, max_profitability
)
