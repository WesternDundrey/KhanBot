    ...
    def add_tree_view(self):
        with mui.lab.TreeView(
            ...
            with mui.lab.TreeItem(
                nodeId="directional_strategies", label="⚔️Directional Strategies"
            ):
                for controller in available_controllers:
                    if (
                        available_controllers[controller]["type"]
                        == "directional_trading"
                    ):
                        mui.lab.TreeItem(
                            nodeId=constants.CONTROLLERS_PATH
                            + "/"
                            + controller
                            + ".py",
                            label=f"🐍{controller}",
            ...
            with mui.lab.TreeItem(
                nodeId="market_making_strategies", label="🪙Market Making Strategies"
            ):
                for controller in available_controllers:
