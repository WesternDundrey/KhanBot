...
class BotPerformanceCardV2(Dashboard.Item):
    DEFAULT_COLUMNS = [
        ...
        {
            "field": "realized_pnl_quote",
            "headerName": "Realized PNL ($)",
            "width": MEDIUM_COL_WIDTH,
            "editable": False,
        },
        {
            "field": "unrealized_pnl_quote",
            "headerName": "Unrealized PNL ($)",
            "width": MEDIUM_COL_WIDTH,
            "editable": False,
        },
        {
            "field": "global_pnl_quote",
            "headerName": "NET PNL ($)",
            "width": MEDIUM_COL_WIDTH,
            "editable": False,
        },
        ...
        {
            "field": "open_order_volume",
            "headerName": "Liquidity Placed ($)",
            "width": MEDIUM_COL_WIDTH,
            "editable": False,
        },
        {
        ...
        {
            "field": "close_types",
            "headerName": "Close Types",
            "width": ULTRA_WIDE_COL_WIDTH,
            "editable": False,
        },
    ]
            ...
            if bot_status.get("status") == "error":
                ...
                if is_running:
                    for controller, inner_dict in performance.items():
                        ...
                        kill_switch_status = (
                            True
                            if controller_config.get("manual_kill_switch") is True
                            else False
                        )
                        realized_pnl_quote = controller_performance.get(
                            "realized_pnl_quote", 0
                        )
                        unrealized_pnl_quote = controller_performance.get(
                            "unrealized_pnl_quote", 0
                        )
                        global_pnl_quote = controller_performance.get(
                            "global_pnl_quote", 0
                        )
                        volume_traded = controller_performance.get("volume_traded", 0)
                        open_order_volume = controller_performance.get(
                            "open_order_volume", 0
                        )
                        imbalance = controller_performance.get("inventory_imbalance", 0)
                        close_types = controller_performance.get(
                            "close_type_counts", {}
                        )
                ...
                total_global_pnl_pct = (
                    total_global_pnl_quote / total_volume_traded
                ...
                with mui.Card(
                    ...
                    mui.CardHeader(
                        ...
                        action=(
                            mui.IconButton(
                                mui.icon.Stop, onClick=lambda: stop_bot(bot_name)
                            )
                            if is_running
                            else mui.IconButton(
                        ...
                        with mui.CardContent(sx={"flex": 1}):
                                ...
                                with mui.Grid(item=True, xs=2):
                                    with mui.Paper(
                                        ...
                                        with self.title_bar(
                                            ...
                                            mui.Typography(
                                                "💸 Volume Traded", variant="h6"
                                            )
                                        mui.Typography(
                                            f"$ {total_volume_traded:.2f}",
                                            variant="h6",
                                            sx={"padding": "10px 15px 10px 15px"},
                                        )
                                with mui.Grid(item=True, xs=2):
                            ...
                            with mui.Grid(
                                ...
                                with mui.Grid(item=True, xs=1):
                                    with mui.Button(
                                        onClick=lambda x: self.stop_active_controllers(
                                            bot_name
                                        ),
                                        variant="outlined",
                                        color="warning",
                            ...
                            if len(stopped_controllers_list) > 0:
                                with mui.Grid(
                                    ...
                                    with mui.Grid(item=True, xs=1):
                                        with mui.Button(
                                            onClick=lambda x: self.start_controllers(
                                                bot_name
                                            ),
                                            variant="outlined",
                                            color="success",
                            ...
                            if len(error_controllers_list) > 0:
                                with mui.Grid(
                                    ...
                                    with mui.Grid(item=True, xs=1):
                                        with mui.Button(
                                            onClick=lambda x: self.stop_errors_controllers(
                                                bot_name
                                            ),
                                            variant="outlined",
                                            color="warning",
                            ...
                            with mui.Accordion(sx={"padding": "10px 15px 10px 15px"}):
                                with mui.AccordionSummary(
                                    expandIcon=mui.icon.ExpandMoreIcon()
                                ):
                                ...
                                with mui.AccordionDetails(
                                    ...
                                    if len(error_logs) > 0:
                                        for log in error_logs[:50]:
                                            ...
                                            mui.Typography(
                                                f"{timestamp} - {logger_name}: {message}"
                                            )
                                    else:
