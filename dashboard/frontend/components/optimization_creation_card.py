...
class OptimizationCreationCard(Dashboard.Item):
    ...
    def _create_optimization(self, strategy_info):
        ...
        save_file(
            name=f"{self._strategy_name.lower()}_v_{self._optimization_version}.py",
    ...
    def __call__(self):
        ...
        strategy_names = [
            strategy
            for strategy, strategy_info in available_strategies.items()
            if strategy_info["type"] == "directional_trading"
        ]
        with mui.Paper(
            key=self._key,
            sx={
            ...
            if len(strategy_names) == 0:
                mui.Alert(
                    "No strategies available, please create one to optimize it",
                    severity="warning",
                    sx={"width": "100%"},
                )
                return
                ...
                with mui.Grid(container=True, spacing=2, sx={"padding": "10px"}):
                    ...
                    with mui.Grid(item=True, xs=4):
                        with mui.FormControl(variant="standard", sx={"width": "100%"}):
                            mui.TextField(
                                defaultValue=self._optimization_version,
                                label="Optimization version",
                                variant="standard",
                                onChange=lazy(self._set_optimization_version),
                            )
                    with mui.Grid(item=True, xs=4):
                        with mui.Button(
                            variant="contained",
                            onClick=lambda x: self._create_optimization(
                                available_strategies[self._strategy_name]
                            ),
                            sx={"width": "100%"},
                        ):
                            mui.icon.Add()
