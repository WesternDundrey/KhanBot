...
def get_market_making_general_inputs(custom_candles=False):
    with st.expander("General Settings", expanded=True):
        ...
        position_mode = (
            0 if default_config.get("position_mode", "HEDGE") == "HEDGE" else 1
        )
        cooldown_time = default_config.get("cooldown_time", 60 * 60) / 60
        executor_refresh_time = (
            default_config.get("executor_refresh_time", 60 * 60) / 60
        )
        ...
        with c6:
            cooldown_time = (
                st.number_input(
                    "Stop Loss Cooldown Time (minutes)",
                    value=cooldown_time,
        ...
        with c7:
            executor_refresh_time = (
                st.number_input(
                    "Executor Refresh Time (minutes)",
                    value=executor_refresh_time,
                    help="Enter the refresh time in minutes for executors (e.g., 60).",
                )
                * 60
            )
        if custom_candles:
