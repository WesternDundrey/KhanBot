...
class LaunchStrategyV2(Dashboard.Item):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._controller_configs_available = (
            self._backend_api_client.get_all_controllers_config()
        )
    ...
    def launch_new_bot(self):
        ...
        if not self._image_name:
            ...
            return
        if (
            not self._controller_config_selected
            or len(self._controller_config_selected) == 0
        ):
            st.warning(
    ...
    def __call__(self):
        with mui.Paper(
            ...
            with mui.Grid(
                container=True, spacing=2, sx={"padding": "10px 15px 10px 15px"}
            ):
                ...
                for config in all_controllers_config:
                    ...
                    if len(config_version) > 1:
                        ...
                        version = "NaN"
                    ts_text = (
                        str(trailing_stop["activation_price"])
                        + " / "
                ...
                with mui.Grid(item=True, xs=12):
                    with mui.Paper(
                        ...
                        with self.title_bar(
                            padding="10px 15px 10px 15px", dark_switcher=False
                        ):
