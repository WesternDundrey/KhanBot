...
def get_executors_distribution_inputs(use_custom_spread_units=False):
    ...
    if use_custom_spread_units:
        buy_spreads = [
            spread / 100 for spread in default_config.get("buy_spreads", [1, 2])
        ]
        sell_spreads = [
            spread / 100 for spread in default_config.get("sell_spreads", [1, 2])
        ]
    else:
    ...
    with st.expander("Executors Configuration", expanded=True):
        ...
        with col_sell:
            ...
            sell_order_levels = st.number_input(
                "Number of Sell Order Levels",
                min_value=1,
                value=sell_order_levels_def,
                help="Enter the number of sell order levels (e.g., 2).",
            )
        if buy_order_levels > buy_order_levels_def:
            buy_spreads += [0.01 + max(buy_spreads)] * (
                buy_order_levels - buy_order_levels_def
            )
            buy_amounts_pct += [0.2 + max(buy_amounts_pct)] * (
                buy_order_levels - buy_order_levels_def
            )
        ...
        if sell_order_levels > sell_order_levels_def:
            sell_spreads += [0.01 + max(sell_spreads)] * (
                sell_order_levels - sell_order_levels_def
            )
            sell_amounts_pct += [0.2 + max(sell_amounts_pct)] * (
                sell_order_levels - sell_order_levels_def
            )
        ...
        col_buy_spreads, col_buy_amounts, col_sell_spreads, col_sell_amounts = (
            st.columns(4)
        )
    ...
    all_orders_amount_normalized = normalize(
        buy_amount_distributions + sell_amount_distributions
    )
    buy_order_amounts_pct = [
        amount for amount in all_orders_amount_normalized[:buy_order_levels]
    ]
