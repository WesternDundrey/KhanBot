...
def get_dca_distribution_inputs():
    with st.expander("DCA Builder", expanded=True):
        ...
        ts_ap = (
            default_config.get("trailing_stop", {}).get("activation_price", 0.018) * 100
        )
        ts_delta = (
            default_config.get("trailing_stop", {}).get("trailing_delta", 0.002) * 100
        )
        ...
        with c1:
            ...
            if buy_order_levels > levels_def:
                ...
                dca_amounts += [0.2 + max(dca_amounts)] * (
                    buy_order_levels - levels_def
                )
        ...
        spreads_normalized = [
            spread - spread_distributions[0] for spread in spread_distributions
        ]
        st.write("---")
        ...
        with c2:
            ...
            sl = (
                st.number_input(
                    ...
                    value=sl,
                    step=0.1,
                    help="Enter the stop loss as a percentage (e.g., 2.0 for 2%).",
                )
                / 100
            )
            # with c2:
            tp = (
                st.number_input(
                    ...
                    value=tp,
                    step=0.1,
                    help="Enter the take profit as a percentage (e.g., 3.0 for 3%).",
                )
                / 100
            )
            # with c3:
            time_limit = (
                st.number_input(
                    ...
                    min_value=0,
                    value=time_limit,
                    help="Enter the time limit in minutes (e.g., 360 for 6 hours).",
                )
                * 60
            )
            # with c4:
            ts_ap = (
                st.number_input(
                    ...
                    value=ts_ap,
                    step=0.1,
                    help="Enter the tr  ailing stop activation price as a percentage (e.g., 1.0 for 1%).",
                )
                / 100
            )
            # with c5:
            ts_delta = (
                st.number_input(
                    ...
                    value=ts_delta,
                    step=0.1,
                    help="Enter the trailing stop delta as a percentage (e.g., 0.3 for 0.3%).",
                )
                / 100
            )
