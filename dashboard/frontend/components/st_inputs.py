...
def distribution_inputs(column, dist_type_name, levels=3, default_values=None):
    ...
    if dist_type != "Manual":
        ...
        if default_values:
            manual_values = [
                column.number_input(
                    f"{dist_type_name} for level {i + 1}",
                    value=value * 100.0,
...
def get_distribution(
    ...
    if dist_type == "Manual":
        ...
        distribution = [Decimal("0")] + Distributions.geometric(
            n_levels - 1, start, ratio
        )
