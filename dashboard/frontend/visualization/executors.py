...
def add_executors_trace(fig, executors, row, col):
    for executor in executors:
        ...
        exit_price = executor.custom_info.get(
            "close_price", executor.custom_info["current_position_average_price"]
        )
        name = (
            "Buy Executor" if executor.config.side == TradeType.BUY else "Sell Executor"
        )
