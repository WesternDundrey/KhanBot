...
def display_dca_performance(executors: pd.DataFrame):
    ...
    with col1:
        ...
        level_id_pie_chart_fig.add_trace(
            go.Pie(labels=executors["level_id"], values=executors["id"], hole=0.4)
        )
        st.plotly_chart(level_id_pie_chart_fig, use_container_width=True)
    with col2:
        intra_level_id_data = (
            executors.groupby(["exit_level", "close_type"])
            .size()
        ...
        fig.add_trace(
            go.Pie(
                labels=intra_level_id_data.loc[
                    intra_level_id_data["exit_level"] != 0, "exit_level"
                ],
                values=intra_level_id_data.loc[
                    intra_level_id_data["exit_level"] != 0, "count"
                ],
                hole=0.4,
            )
        )
    ...
    with col3:
        intra_level_id_pnl_data = (
            executors.groupby(["exit_level"])["net_pnl_quote"]
            .sum()
            .reset_index(name="pnl")
        )
        fig = go.Figure()
        for close_type in intra_level_id_data["close_type"].unique():
            temp_data = intra_level_id_data[
                intra_level_id_data["close_type"] == close_type
            ]
            fig.add_trace(
                go.Bar(
        ...
        fig.add_trace(
            go.Scatter(
                x=intra_level_id_pnl_data["exit_level"],
        ...
        max_count = max(
            abs(intra_level_id_data["count"].min()),
            abs(intra_level_id_data["count"].max()),
        )
        max_pnl = max(
            abs(intra_level_id_pnl_data["pnl"].min()),
            abs(intra_level_id_pnl_data["pnl"].max()),
        )
        ...
        fig.update_layout(
            ...
            yaxis2=dict(
                title="PnL", overlaying="y", side="right", range=[-max_pnl, max_pnl]
            ),
            barmode="group",
        )
    ...
    level_id_data[["type", "number"]] = level_id_data["level_id"].str.split(
        "_", expand=True
    )
    ...
    fig = go.Figure()
    level_id_pie_chart_traces = go.Bar(
        x=level_id_data["level_id"], y=level_id_data["id"]
    )
