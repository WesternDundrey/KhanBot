...
def get_volume_trace(df):
    ...
    return go.Bar(
        ...
        y=df["volume"],
        name="Volume",
        marker_color=theme.get_color_scheme()["volume"],
        opacity=0.7,
    )
...
def get_macd_traces(df, macd_fast, macd_slow, macd_signal):
    ...
    traces = [
        go.Scatter(
            x=df.index,
            y=df[macd],
            line=dict(color=tech_colors["macd_line"]),
            name="MACD Line",
        ),
        go.Scatter(
            x=df.index,
            y=df[macd_s],
            line=dict(color=tech_colors["macd_signal"]),
            name="MACD Signal",
        ),
        go.Bar(
            x=df.index,
    ...
    for i in range(len(df)):
        if i == 0 or df[supertrend_d].iloc[i] == df[supertrend_d].iloc[i - 1]:
            ...
            current_segment["color"] = (
                tech_colors["buy"]
                if df[supertrend_d].iloc[i] == 1
                else tech_colors["sell"]
            )
        else:
