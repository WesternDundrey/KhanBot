...
def render_backtesting_metrics(summary_results, title="Backtesting Metrics"):
    ...
    total_executors_with_position = summary_results.get(
    ...
    col1.metric(
        label="Net PNL (Quote)", value=f"{net_pnl_quote:.2f}", delta=f"{net_pnl:.2%}"
    )
    col2.metric(
        label="Max Drawdown (USD)",
        value=f"{max_drawdown_usd:.2f}",
        delta=f"{max_drawdown_pct:.2%}",
    )
    ...
    col6.metric(
        label="Total Executors with Position", value=total_executors_with_position
    )
