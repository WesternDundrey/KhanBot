...
def create_executors_distribution_traces(
    ...
    buy_order_amounts_quote = [
        amount * total_amount_quote for amount in buy_amounts_pct
    ]
    sell_order_amounts_quote = [
        amount * total_amount_quote for amount in sell_amounts_pct
    ]
    ...
    for i, value in enumerate(buy_order_amounts_quote):
        fig.add_annotation(
            ...
            y=value
            + 0.03
            * max(buy_order_amounts_quote),  # Offset the text slightly above the bar
    ...
    for i, value in enumerate(sell_order_amounts_quote):
        fig.add_annotation(
            ...
            y=value
            + 0.03
            * max(sell_order_amounts_quote),  # Offset the text slightly above the bar
            text=str(round(value, 2)),
            showarrow=False,
