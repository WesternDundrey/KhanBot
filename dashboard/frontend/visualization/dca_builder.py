...
def create_dca_graph(dca_inputs, dca_amount):
    ...
    dca_order_amounts = [
        amount_dist * dca_amount for amount_dist in dca_inputs["dca_amounts"]
    ]
        ...
        break_even = (
            (
                sum(
                    [
                        spread * amount
                        for spread, amount in zip(dca_spreads_normalized, amounts)
                    ]
                )
                / sum(amounts)
            )
    ...
    accumulated_amount = [
        sum(dca_order_amounts[: i + 1]) for i in range(len(dca_order_amounts))
    ]

    # Calculate unrealized PNL
    cum_unrealized_pnl = calculate_unrealized_pnl(
        dca_spreads, break_even_values, accumulated_amount
    )
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker=dict(
                color="#FFA07A", opacity=0.6
            ),  # Light Salmon color, adjust as needed
        ),
        secondary_y=True,
    )
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=dca_order_amounts,
            text=[
                f"{amt:.2f}" for amt in dca_order_amounts
            ],  # List comprehension to format text labels
            textposition="auto",
            textfont=dict(color="white", size=12),
            name="Order Amount",
    ...
    fig.add_trace(
        go.Bar(
            ...
            text=[
                f"{amt:.2f}" for amt in accumulated_amount
            ],  # List comprehension to format text labels
            textposition="auto",
            textfont=dict(color="white", size=12),
            name="Cum Amount",
    ...
    fig.add_hline(
        y=last_break_even,
        line_dash="dash",
        annotation_text=f"Global Break Even: {last_break_even:.2f} (%)",
    ...
    profit_per_level = [
        cum_amount * dca_inputs["take_profit"] for cum_amount in accumulated_amount
    ]
    ...
    y_offset_secondary = (
        max_secondary_value * 0.1
    )  # Adjusts the height relative to the maximum value on the secondary y-axis

    # Add annotations to the Plotly figure for the secondary y-axis
    for i, loot in enumerate(loots_to_recover):
        fig.add_annotation(
            x=i,
            y=max_secondary_value
            + y_offset_secondary,  # Position above the maximum value using the offset
    ...
    fig.add_annotation(
        x=max(len(dca_inputs["dca_spreads"]), len(break_even_values))
        - 1,  # Positioning the annotation to the right
