...
class PerformanceDataSource:
    def __init__(self, checkpoint_data: Dict[str, Any]):
        ...
        self.executors_with_orders = self.get_executors_with_orders(
            self.get_executors_df(), self.orders
        )
    ...
    def get_executors_df(
        ...
        executors_df["sl"] = (
            executors_df["config"].apply(lambda x: x.get("stop_loss")).fillna(0)
        )
        executors_df["tp"] = (
            executors_df["config"].apply(lambda x: x.get("take_profit")).fillna(0)
        )
        executors_df["tl"] = (
            executors_df["config"].apply(lambda x: x.get("time_limit")).fillna(0)
        )
        executors_df["close_type_name"] = executors_df["close_type"].apply(
            lambda x: self.get_enum_by_value(CloseType, x).name
        )
        ...
        executors_df = executors_df.merge(
            controllers[["controller_id", "controller_type", "controller_config"]],
            on="controller_id",
            how="left",
        )
    ...
    def get_executors_with_orders(executors_df: pd.DataFrame, orders: pd.DataFrame):
        ...
        exec_with_orders = df.merge(
            orders, left_on="order_id", right_on="client_order_id", how="inner"
        )
        exec_with_orders = exec_with_orders[
            exec_with_orders["last_status"].isin(
                ["SellOrderCompleted", "BuyOrderCompleted"]
            )
        ]
        return exec_with_orders[
            [
                "executor_id",
                "order_id",
                "last_status",
    ...
    def get_executor_info_list(
        self, executors_filter: Dict[str, Any] = None
    ) -> List[ExecutorInfo]:
        required_columns = [
            "id",
            "timestamp",
        ...
        executors_df = self.get_executors_df(
        ...
        executor_info_list = executors_df.apply(
            lambda row: ExecutorInfo(**row.to_dict()), axis=1
        ).tolist()
    ...
    def get_executor_dict(
        ...
        executors_df = self.get_executors_df(
            executors_filter, apply_executor_data_types=apply_executor_data_types
        ).copy()
    ...
    def get_executors_by_controller_type(
        self, executors_filter: Dict[str, Any] = None
        ...
        for key, value in filters.items():
            if isinstance(value, list) and len(value) > 0:
                ...
                filter_condition &= np.array(
                    executors_df["close_timestamp"] <= value + 60
                )
