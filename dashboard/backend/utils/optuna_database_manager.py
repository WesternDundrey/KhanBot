...
class OptunaDBManager:
        ...
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM trials"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_studies_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM studies"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_trial_params_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM trial_params"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_trial_values_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM trial_values"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_trial_system_attributes_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM trial_system_attributes"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_version_info_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM version_info"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_study_directions_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM study_directions"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_study_user_attributes_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM study_user_attributes"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_study_system_attributes_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM study_system_attributes"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_trial_user_attributes_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM trial_user_attributes"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_trial_intermediate_values_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM trial_intermediate_values"),
                    session.connection(),
                )
            return df
        except Exception as e:
    ...
    def _get_trial_heartbeats_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM trial_heartbeats"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_alembic_version_table(self):
        try:
            with self.session_maker() as session:
                df = pd.read_sql_query(
                    text("SELECT * FROM alembic_version"), session.connection()
                )
            return df
        except Exception as e:
            return f"Error: {str(e)}"
    ...
    def _get_merged_df(self):
        ...
        merged_df = (
            self.trials.merge(self.studies, on="study_id")
            .merge(
                pd.pivot(
                    self.trial_user_attributes,
                    index="trial_id",
                    columns="key",
                    values="value_json",
                ),
